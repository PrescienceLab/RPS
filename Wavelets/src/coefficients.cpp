...
// N=2
const double daub_g2[2] = {1.0/2,
			   1.0/2};

// N=4
const double daub_g4[4] = {(1+sqrt(3))/(4*2),
			   (3+sqrt(3))/(4*2),
			   (3-sqrt(3))/(4*2),
			   (1-sqrt(3))/(4*2)};
...
// N=6
const double K6 = sqrt(5+2*sqrt(10));
const double daub_g6[6] = {(1+sqrt(10)+K6)/(16*2),
			   (5+sqrt(10)+3*K6)/(16*2),
			   (5-sqrt(10)+K6)/(8*2),
			   (5-sqrt(10)-K6)/(8*2),
			   (5+sqrt(10)-3*K6)/(16*2),
...
// N=8
const double daub_g8[8] = {+0.325803428051/2,
			   +1.010945715092/2,
			   +0.892200138246/2,
			   -0.039575026236/2,
			   -0.264507167369/2,
...
// N=10
const double daub_g10[10] = {+0.226418982583/2,
			     +0.853943542705/2,
			     +1.024326944260/2,
			     +0.195766961347/2,
			     -0.342656715382/2,
...
// N=12
const double daub_g12[12] = {+0.157742432003/2,
			     +0.699503814075/2,
			     +1.062263759882/2,
			     +0.445831322930/2,
			     -0.319986598891/2,
...
// N=14
const double daub_g14[14] = {+0.110099430746/2,
			     +0.560791283626/2,
			     +1.031148491636/2,
			     +0.664372482211/2,
			     -0.203513822463/2,
...
// N=16
const double daub_g16[16] = {+0.076955622108/2,
			     +0.442467247152/2,
			     +0.955486150427/2,
			     +0.827816532422/2,
			     -0.022385735333/2,
...
// N=18
const double daub_g18[18] = {+0.053850349589/2,
			     +0.344834303815/2,
			     +0.855349064359/2,
			     +0.929545714366/2,
			     +0.188369549506/2,
...
// N=20
const double daub_g20[20] = {+0.037717157593/2,
			     +0.266122182794/2,
			     +0.745575071487/2,
			     +0.973628110734/2,
			     +0.397637741770/2,
...
void WaveletCoefficients::GetTransformCoefsLPF(vector<double> &coefs)
{
  ...
  for (unsigned i=0; i<numcoefs; i++) {
    coefs.push_back(g_coefs[numcoefs-1-i]);
  }
}
...
void WaveletCoefficients::GetTransformCoefsHPF(vector<double> &coefs)
{
  assert(coefs.size()==0);
  for (unsigned i=0; i<numcoefs; i++) {
    coefs.push_back(h_coefs[numcoefs-1-i]);
  }
}
