...
int main(int argc, char *argv[])
{
  ...
  if (numstages <= 0) {
    cerr << "block_dynamic_mixed_srwt: Number of stages must be positive.\n";
    exit(-1);
  }
  ...
  if (toupper(argv[8][0])=='N') {
    ...
    cerr << "block_dynamic_mixed_srwt: Need to choose flat or noflat for human readable.\n";
    exit(-1);
  }
  ...
  while ( fp.ParseMRACoefsBlock(optim_spec, approxcoefs, detailcoefs, *is,
    ...
    if (approxcoefs.size()) {
      if (approxcoefs[0].GetBlockSize() > 0) {
	unsigned approxlevel=approxcoefs[0].GetBlockLevel();
	wavecoefs[approxlevel+1] = approxcoefs[0];
	wavecoefs[approxlevel+1].SetBlockLevel(approxlevel+1);
      }
    }

    i=0;
    for (i=0; i<detailcoefs.size(); i++) {
    ...
    if (transform) {
      drwt.StreamingTransformBlockOperation(reconst, dlysamples);
    } else {
      vector<int> zerospec;
      vector<int> specs;
      FlattenSignalSpec(specs, optim_spec);
      InvertSignalSpec(zerospec, specs, optim_stages+1, 0);
      drwt.StreamingTransformZeroFillBlockOperation(reconst, dlysamples, zerospec);
    }
    ...
    for (i=0; i<approxcoefs.size(); i++) {
      approxcoefs[i].ClearBlock();
    }
    for (i=0; i<detailcoefs.size(); i++) {
      detailcoefs[i].ClearBlock();
    }
    for (i=0; i<optim_stages+1; i++) {
      wavecoefs[i].ClearBlock();
    }
    reconst.ClearBlock();
