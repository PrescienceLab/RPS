...
$usage = "run_sched_experiment.pl [--aggressive] setupfile numsamples minint:maxint mindur:maxdur minslack:maxslack";

&GetOptions("aggressive"=>\$aggressive) and $#ARGV==4 or die "usage: $usage\n";
...
$outputfile = $aggressive ? "sched_aggressive_" : "sched_";
$outputfile.="time${timestamp}_${setupname}_int${minint}:${maxint}_dur${mindur}:${maxdur}_slack${minslack}:${maxslack}.out";

$call = $aggressive ? "test_sched_aggressive" : "test_sched";
$call.=" $numsamples 0.95 $minint:$maxint $mindur:$maxdur $minslack:$maxslack $numhosts 3 ";
