...
// xx>0 is required
inline double LnGammaPos(double xx)
{
...
inline double GammaPdf(double alpha, double omega, double x)
{
  if (x<0.0) {
    return 0.0;
  } else {
...
inline double GammaCdf(double alpha, double omega, double x)
{
  double val, sum;
  int i;
...
inline double ChiSquaredPdf(int r, double x)
{
  return GammaPdf((double)r/2,2.0,x);
}

inline double ChiSquaredCdf(int r, double x)
{
...
inline double ConfToSigmas(double conf)
{
  return sqrt(2)*ErfInv(conf);
}

inline double SigmasToConf(double sigmas)
{
